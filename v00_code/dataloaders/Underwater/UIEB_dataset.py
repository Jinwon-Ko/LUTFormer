...
class UIEB_train_dataset(Dataset):
    def __init__(self, cfg):
        ...
        root_dir = os.path.join(cfg.dataset_root, 'Underwater/UIEB')

        datalist_txt = os.path.join(root_dir, f'train_list.txt')
        with open(datalist_txt, 'r') as fid:
            datanames = fid.readlines()
        datanames = [dataname.strip() for dataname in datanames]

        img_dir = os.path.join(root_dir, 'input')
        gt_dir = os.path.join(root_dir, f'GT')
        datalist = os.listdir(img_dir)

        self.img_list = list()
        self.gt_list = list()
        for img_name in datalist:
            if img_name in datanames:
...
class UIEB_test_dataset(Dataset):
    def __init__(self, cfg):
        self.cfg = cfg

        root_dir = os.path.join(cfg.dataset_root, f'Underwater/UIEB')

        datalist_txt = os.path.join(root_dir, f'test_list.txt')
        with open(datalist_txt, 'r') as fid:
            datanames = fid.readlines()
        datanames = [dataname.strip() for dataname in datanames]
